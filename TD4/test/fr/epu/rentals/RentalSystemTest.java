...
class RentalSystemTest {
    ...
    @org.junit.jupiter.api.Test
    void testARental() throws NoSuchItemException {
        initRentalSystem();
        costumeRentalSystem.rentItem(costumeBM, today, tomorrow.plusDays(1));
        Rental rental = (Rental) costumeRentalSystem.getRentals().get(0);
    ...
    // Ajoutez un test, si vous ne l’avez pas déjà, pour vérifier que l’exception est bien levée
    //quand l’objet est inconnu.

    @org.junit.jupiter.api.Test
    void testIsRentableWithException() throws NoSuchItemException {
        initRentalSystem();
        assertThrows(NoSuchItemException.class, () -> costumeRentalSystem.isRentable(new Costume("Spiderman", "rouge", CostumeSize.LARGE), today, tomorrow));
    }
